...
/** When empty, no image/emoji/bootstrap icon is shown (placeholders removed). */
function hasPageIcon(icon) {
    return Boolean(icon && String(icon).trim());
}

function pageIconSidebarHtml(icon) {
    if (!hasPageIcon(icon)) return "";
    if (isFileOrLink(icon)) {
        return `<div class="icon"><img src="${icon}" class="tw-object-contain" alt="" /></div>`;
    }
    if (isEmoji(icon)) {
        return `<p class="">${icon}</p>`;
    }
    return `<i class="${icon}"></i>`;
}

function pageIconContentHtml(icon) {
    if (!hasPageIcon(icon)) return "";
    if (isFileOrLink(icon)) {
        return `<img src="${icon}" class="tw-object-cover tw-w-full tw-h-full" alt="" />`;
    }
    if (isEmoji(icon)) {
        return `<p class="">${icon}</p>`;
    }
